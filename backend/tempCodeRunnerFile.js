...
const connectDB = async () => {
    try {
        await mongoose.connect('mongodb+srv://prempatel1248:7vkxIF9AlSXZ951r@cluster0.pky1g.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0');
...
app.post('/logActivity', async (req, res) => {
    ...
    try {
        ...
        await User.updateOne(
            { email },
            { $push: { logs: log } } // push log object into the logs array
        );
...
app.get('/logs', async (req, res) => {
    try {
        ...
        // Flatten logs and include user email and mediaType
        const allLogs = users.flatMap(user =>
            user.logs.map(log => ({
                ...
                message: log.message,
                time: log.time,
                mediaType: log.mediaType || "Text" // default to Text if not set
            }))
        );
